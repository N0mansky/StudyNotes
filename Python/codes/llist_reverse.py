...
# First method,modify the val of nodes
def pair_rev(head):
...
def pair_rev2(head):
    if head != None and head.pt != None:
        pt = head.pt
        head.pt = pair_rev2(pt.pt)
        pt.pt = head
        return pt
    return head
...
class TestLink(unittest.TestCase):
    ...
    def test_pair_rev2(self):
        self.head1 = pair_rev2(self.head1)
        self.head2 = pair_rev2(self.head2)
        rst1 = self.prt_link(self.head1)
        rst2 = self.prt_link(self.head2)
        self.assertEqual(rst1, self.expected1)
        self.assertEqual(rst2, self.expected2)
